...
struct EnvironmentObjectViewContainer<V: View>: View {
    ...
    /// This initialiser is to be used in the case where the content depends on the info contained in the levels model
    init(@ViewBuilder _ content: (LevelsModel) -> V) {
        var model = LevelsModel()
        guard let levelsBundleUrl = Bundle.main.url(forResource: "levels", withExtension: "json")
        else {
            // error out
            self.model = LevelsModel()
            self.content = content(LevelsModel())
            return
            //            throw AssetError.notFound
        }
